...
public class SwerveModuleGroup {
  ...
  protected ModuleIO[] io = new ModuleIO[4];
  protected ModuleIOInputsAutoLogged[] swerveModuleInputs = new ModuleIOInputsAutoLogged[4];
  ...
  public SwerveModuleGroup(SwerveModuleConstants config, ModuleIO... modules) {
    ...
    for (int i = 0; i < moduleCount; i++) {
      this.io[i] = modules[i];
      this.swerveModuleInputs[i] = new ModuleIOInputsAutoLogged();
  ...
  public void setStates(SwerveModuleState... currentStates) {
    ...
    for (int i = 0; i < moduleCount; i++) {
      this.io[i].setState(currentStates[i]);
    }
  }
  ...
  public SwerveModulePosition[] getPositions(boolean refresh) {
    return new SwerveModulePosition[] {
            io[0].getPosition(refresh),
            io[1].getPosition(refresh),
            io[2].getPosition(refresh),
            io[3].getPosition(refresh)
    };
  }
  ...
  public SwerveModuleState[] getStates(boolean refresh) {
    return new SwerveModuleState[] {
            io[0].getState(refresh),
            io[1].getState(refresh),
            io[2].getState(refresh),
            io[3].getState(refresh)
    };
  }
...
//
    // Get the change in the simulated swerve module position
        for(int i = 0; i < moduleCount; i++){
            deltaPositions[i] = io[i].getModuleDelta();
        }
